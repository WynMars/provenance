...
// todo - start the endpoint worker
            ...
            EndpointWorkFinder endpointWorkFinder = new EndpointWorkFinder(endpointDataGateway);
            RestTemplate restTemplate = new RestTemplate();
            EndpointWorker endpointWorker = new EndpointWorker(restTemplate, articleDataGateway);
            List<Worker<EndpointTask>> workers = Collections.singletonList(endpointWorker);
            WorkScheduler<EndpointTask> scheduler = new WorkScheduler<>(endpointWorkFinder, workers, 300);
            scheduler.start();
